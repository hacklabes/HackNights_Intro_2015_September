...
/* incoming osc message are forwarde d to the oscEvent method. */
void oscEvent(OscMessage theOscMessage) {
  ...
  if(theOscMessage.addrPattern().equals("/leftVolume")){
    posYPad = map(v, 0,128, 0,height);
  }else if(theOscMessage.addrPattern().equals("/rightVolume")){
    //pad2Pos.y = map(v, 0,128, height,0);
  }
